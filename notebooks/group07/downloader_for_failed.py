...
OUTPUT_FILE = "/mnt/group07/final_data_product/downloaded_images-retry-final_v2.parquet"
# INPUT_FILE = "/mnt/group07/final_data_product/classification_input.parquet"
INPUT_FILE = "/mnt/group07/final_data_product/download_input-retry.parquet"
...
def download_and_save_image(row):
  ...
  if row.longitude is not None:
    try:
      ...
      buffer = io.BytesIO()
      # Convert to RGB to fix error (POST FIX)
      img = img.convert('RGB')
...
df_output = spark.read.parquet(OUTPUT_FILE)

# COMMAND ----------

df_output.groupBy("error_code").count().show(truncate=False)
